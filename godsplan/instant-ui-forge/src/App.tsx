...
// Fallback rich mock database - used instantly on first paint, then
// replaced by real Places/Colleges from the Django + Postgres backend
// once fetchDiscoveryNodes() resolves (see useEffect below). If the API
// is unreachable (e.g. backend not running yet) the app keeps working
// with this data so the UI is never blank.
...
export default function App() {
  ...
  // Live data from the Django + Postgres backend. Starts with the fallback
  // mock set so the UI renders instantly, then swaps to real Places/Colleges
  // as soon as the API responds (see useEffect below).
    ...
    fetchDiscoveryNodes()
      .then((liveNodes) => {
        if (!cancelled && liveNodes.length > 0) {
          setNodes(liveNodes as Node[]);
          setIsBackendConnected(true);
        }
      })
      .catch(() => {
        // Backend not reachable yet (e.g. not configured / not running) -
        // keep using FALLBACK_NODES, no error shown to the user.
  ...
  // Account sync (real Django auth) - lives alongside the fictional
  // "calibration" persona system below; this is the real login/register
  // that both the Web and App frontends share against the same backend.
  ...
  const handleRegister = async () => {
    ...
    setAccountStatus("");
    try {
      const user = await registerUser(accountUsername, accountEmail, accountPassword);
      setLoggedIn(true);
      setUserName(user.username);
      setAccountStatus(`Account created. Signed in as ${user.username}.`);
    } catch (e: any) {
      setAccountStatus(e.message || "Registration failed.");
    } finally {
  ...
  const handleLogin = async () => {
    ...
    setAccountStatus("");
    try {
      const res = await loginUser(accountUsername, accountPassword);
      setLoggedIn(true);
      setUserName(res.username);
      setAccountStatus(`Signed in as ${res.username}.`);
    } catch (e: any) {
      setAccountStatus(e.message || "Login failed.");
    } finally {
      setAccountBusy(false);
    }
  };
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      /* token already invalid - fine either way */
    } finally {
      setLoggedIn(false);
  ...
  const [activeMapNode, setActiveMapNode] = useState<Node | null>(FALLBACK_NODES[0]);
          ...
          <div className="space-y-10 animate-fade-in">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {nodes.filter(n => n.category === "culinary" && (filterType === "all" || n.type === filterType) && n.distance <= filterDistance).length === 0 && (
          ...
          <div className="space-y-10 animate-fade-in">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="lg:col-span-5 space-y-6">
                ...
                {/* REAL Backend Account Sync (Django + Postgres auth) */}
                <div className="bg-[#111] border border-white/10 p-5 space-y-4">
                  <div className="flex justify-between items-center border-b border-white/5 pb-2">
                    <h3 className="text-xs uppercase tracking-[0.25em] font-black text-white/80 font-syne">
                      Account Sync (Cloud Database)
                    </h3>
                    <span className={`text-[8px] uppercase font-bold px-2 py-1 border ${isBackendConnected ? "border-green-500 text-green-400" : "border-white/20 text-white/40"}`}>
                      {isBackendConnected ? "DB Connected" : "Offline Mode"}
                    </span>
                  </div>

                  {loggedIn ? (
                    <div className="space-y-3">
                      <p className="text-[10px] text-white/60 font-space">
                        Signed in. This account is shared with the GodsPlan App (mobile) - saved places and reviews sync both ways through the same database.
                      </p>
                      <button
                        id="account-logout-btn"
                        disabled={accountBusy}
                        onClick={handleLogout}
                        className="w-full bg-transparent border border-white/20 text-white text-[10px] uppercase font-bold tracking-widest py-2.5 hover:border-white transition-colors cursor-pointer disabled:opacity-40"
                      >
                    ...
                    <div className="space-y-3">
                      <input
                        ...
                        className="w-full bg-[#1A1A1A] text-white text-xs px-3 py-2.5 border border-white/15 focus:outline-none focus:border-white font-space"
                      />
                      <input
                        ...
                        placeholder="Email (needed for Register)"
                        className="w-full bg-[#1A1A1A] text-white text-xs px-3 py-2.5 border border-white/15 focus:outline-none focus:border-white font-space"
                      />
                      <input
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          disabled={accountBusy}
                          onClick={handleLogin}
                          className="flex-1 bg-white text-black text-[10px] uppercase font-bold tracking-widest py-2.5 hover:bg-neutral-300 transition-colors cursor-pointer disabled:opacity-40"
                        >
                          Sign In
                        </button>
                        <button
                          ...
                          className="flex-1 bg-transparent border border-white/30 text-white text-[10px] uppercase font-bold tracking-widest py-2.5 hover:border-white transition-colors cursor-pointer disabled:opacity-40"
                        >
                          Register
                        </button>
                      </div>
                  ...
                  {accountStatus && (
                    <p className="text-[9px] text-white/50 font-space pt-1 border-t border-white/5">{accountStatus}</p>
                  )}
                </div>
        ...
        <div className="fixed inset-0 bg-black/85 backdrop-blur-xs flex items-center justify-center p-6 z-50">
          <div className="bg-[#0D0D0D] border border-white/25 max-w-lg w-full p-6 md:p-8 relative">
            ...
            <div className="mt-8 pt-4 border-t border-white/5 flex gap-3">
              ...
              {(selectedNode.id.startsWith("place-") || selectedNode.id.startsWith("college-")) && (
                <button
                  id="modal-review-btn"
                  onClick={async () => {
                    if (!loggedIn) {
                      alert("Sign in first (Profile tab) to leave a review.");
                      return;
                    }
                    const ratingStr = window.prompt("Rate 1-5:", "5");
                    if (!ratingStr) return;
                    const text = window.prompt("Your review:", "") || "";
                    try {
                      await submitReview(selectedNode.id, Number(ratingStr), text);
                      alert("Review submitted - visible to both Web and App users.");
                    } catch (e: any) {
              ...
              {(selectedNode.id.startsWith("place-") || selectedNode.id.startsWith("college-")) && (
                <button
                  ...
                  onClick={async () => {
                    if (!loggedIn) {
                      alert("Sign in first (Profile tab) to save nodes to your account.");
                      return;
                    }
                    const ok = await toggleSavedNode(selectedNode.id);
                    alert(ok ? "Saved to your GodsPlan account (visible on the App too)." : "Could not save right now.");
                  }}
                  className="border border-white/20 text-white/70 hover:text-white px-5 py-3 text-[10px] uppercase font-bold tracking-widest cursor-pointer hover:bg-white/5"
                >
                  Save
